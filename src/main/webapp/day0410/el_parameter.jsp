...
<div>
<!-- empty 값이 비어있니? -->
${ empty param.name ?'아래의 링크를 클릭해주세요':'클릭하셨습니다.' }
</div>
이름 : <strong><%=request.getParameter("name") %> / el 사용 : ${ param.name }</strong>
나이 : <strong><%=request.getParameterValues("age")[0] %> <%=request.getParameterValues("age")[1] %> 
/ el 사용 : ${ paramValues.age[0] }, ${ paramValues.age[1] }</strong>


<!-- 현재 페이지로 name=kim을 age로 20, age로 25를 넘기는 
요청 URL과 QueryString을 만들어보세요.
 -->
<a href="el_parameter.jsp?name=kim&age=20&age=25">요청 URL과 QueryString</a>
</div>
